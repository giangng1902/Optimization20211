...
from solver import heuristic_greedy
    ...
    def create_population(pop_size, old_generation, method):
        ...
        if method == 'random':
            ...
            for _ in range(pop_size):
                ...
                parent3 = rd.choice(old_generation[300:350])
                child = parent1.mates(parent2, parent3)
                offsprings.append(child)
...
if __name__ == '__main__':
    data = Data.generated_with(N=30, K=4, seed=40)
    import solver
    # solver.ip_solver_dyn_sec(data)
    # solver.local_search(data)
    records = genetic_algo(
        data,
        elitism=True,
        crossover_rate=0.4,
        mutation_rate=0.015,
    ...
    x = np.arange(1, 250+1)
    ax.plot(x, records['avg_fit'], label='average fitness')
    ax.plot(x, records['best_fit'], label='best fitness')
    plt.legend()
